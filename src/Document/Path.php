...
final class Path extends Node implements \JsonSerializable
{
    ...
    public function __construct(
        ...
        $this->head = $head;
        $this->trace = $trace;
        !is_null($put) && $this->__connect($put);
        !is_null($patch) && $this->__connect($patch);
        !is_null($delete) && $this->__connect($delete);
        !is_null($options) && $this->__connect($options);
        !is_null($head) && $this->__connect($head);
        !is_null($trace) && $this->__connect($trace);
        !is_null($get) && $this->__connect($get);
        !is_null($post) && $this->__connect($post);
    }
    ...
    {
        if (!$this->hasOperation($type)) {
            throw new \DomainException("Path {$this->name} does not have '{$type}' operation");
        }
