...
class OpenApiDocument extends Node implements \JsonSerializable, \ArrayAccess
{
    ...
    public function __construct(
        ...
        $this->info = $info;
        !is_null($info) && $this->__connect($info);
        $this->setPaths($paths);
        $this->setServers($servers);
        $this->components = $components;
        !is_null($components) && $this->__connect($components);
        $this->setSecurity($security);
        $this->setTags($tags);
        $this->externalDocs = $externalDocs;
        !is_null($externalDocs) && $this->__connect($externalDocs);
    }
    ...
    public function getPath(string $name): Path
    {
        if (!$this->hasPath($name)) {
            throw new \DomainException("Path {$name} does not exist");
        }
